...
def send_welcome_email_to_new_user(user_email, user_id, reset_token, fullname):
    """Sends a password reset email using Flask-Mail."""
    # Ensure user_id is included as a query parameter in the reset link
    reset_link = f"{app.config['FRONTEND_URL']}/reset-password?token={reset_token}&user_id={user_id}"
    ...
    msg.html = render_template_string("""
        ...
        <p>Your account has been created. Please click on the link below to set your initial password:</p>
        <p><a href="{{ reset_link }}">{{ reset_link }}</a></p>
        <p>This link is valid for 1 hour.</p>
...
def create_user():
    ...
    try:
        ...
        send_welcome_email_to_new_user(email, new_user_id, s.dumps(email, salt='new-user-temp-password'), fullname)
...
def get_users():
    ...
    try:
        cur = conn.cursor()
        # Include password_reset_required in the select statement
        cur.execute("SELECT id, username, email, fullname, role, created_at, password_reset_required FROM users")
        users = cur.fetchall()
        cur.close()
...
def request_password_reset():
    ...
    try:
        ...
        if user_data:
            ...
            token = s.dumps(user_email, salt='new-user-temp-password')
...
def reset_password_confirm():
    ...
    try:
        ...
        token_payload = s.loads(token, salt='new-user-temp-password', max_age=3600) # 1 hour expiry
